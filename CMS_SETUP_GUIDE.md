...
- **Admin dashboard tab.** Your view now has two tabs: **Approval Queue** (the working table, unchanged) and **Dashboard**, split into two sections. **Ticket overview**: total/open/in-progress/clarity-or-reopen/completed counts, a high-priority-and-still-open count, and four charts — status breakdown, priority breakdown, ticket type breakdown, and tickets created by month. **Tagging progress**: the original tagged/approved/awaiting-review/accuracy KPIs plus the per-SPOC "tagged vs total" bar chart and the accuracy donut (Correct / Corrected / Admin-tagged / legacy pre-tagged).
- **View conversation.** Every row (both SPOC and admin views) now has a "View conversation" link next to "Open in Zoho" that opens an in-app popup showing the ticket's full comment thread (who wrote it, when, and the text), pulled live from the same `comments` table the dashboard uses — no need to leave the CMS to read ticket history.
